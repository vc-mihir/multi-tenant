<aside class="w-64 flex flex-col shrink-0" style="background-color: #0f172a; border-right: 1px solid #1e293b;">
    ...
    <div class="flex items-center gap-3 px-6 py-8">
        <div class="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-lg" style="background-color: #10b981; shadow-color: rgba(16, 185, 129, 0.4);">
    ...
    <div class="p-6 border-t border-slate-800">
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white shrink-0 shadow-inner" style="background-color: #10b981;">
                {{ strtoupper(substr(Auth::guard('company')->user()->company_name, 0, 2)) }}
            </div>
            <div class="flex-1 min-w-0">
                <p class="text-sm font-bold text-white truncate leading-none">{{ Auth::guard('company')->user()->company_name }}</p>
