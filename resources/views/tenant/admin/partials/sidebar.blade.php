<aside class="w-64 flex flex-col shrink-0" style="background-color: #0f172a; border-right: 1px solid #1e293b;">
    ...
    <nav class="flex-1 px-4 py-4 space-y-2 overflow-y-auto">
        ...
        <a href="{{ route('tenant.admin.users.index') }}"
            class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm transition-all {{ request()->routeIs('tenant.admin.users.index') ? 'font-bold text-emerald-400 bg-slate-800' : 'font-medium text-slate-400 hover:bg-slate-800 hover:text-white' }}">
            <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
